...
class AnemoiParams:
    def __init__(
        ...
        alpha     : Flystel exponent, coprime with p-1; smallest such exponent if not provided
        ...
        M         : lxl MDS matrix for the linear layer; generated via get_mds(l) if not provided
        ...
        if t is not None:
            if t % 2 != 0:
                raise ValueError(f"t must be even. Got {t}")
            if l is not None and t != 2 * l:
                raise ValueError(f"inconsistent 'l' and 't'. Got l={l} and t={t}, expected t = 2*l")
            l = t // 2
    ...
    def _init_mds(self, max_tries: int = 1000):
